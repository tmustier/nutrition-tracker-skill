...
- [ ] Step 1: Check if the dish exists in `data/dishes/` (organised by venue and generic items).
      - Browse `data/food-data-bank-index.md` to find dishes by name, or search by `id` within the folders.
...
- `data/dishes/` — per-dish YAML files by venue/generic (see `data/food-data-bank-index.md` for dish index).
