...
describe('Easter Egg Webhook Integration', () => {
  ...
  describe('Blocking Easter Egg (person_without_food)', () => {
    it('should trigger person_without_food easter egg for selfie and block nutrition extraction', async () => {
      const userId = mockCtx.from.id;

      // Mock Claude Vision API response - selfie detection
      ...
      nock('https://api.anthropic.com')
        ...
        .reply(200, {
          ...
          content: [{
            type: 'text',
            text: JSON.stringify({
              scene_detection: mockSceneDetection,
              nutrition_data: null
            })
          }],
          model: 'claude-3-5-sonnet-20241022',
      ...
      // Simulate the processImage flow
      ...
      expect(result.source).toBe('claude_vision');
    ...
    it('should not trigger easter egg on cooldown', async () => {
      ...
      expect(secondResult.cooldownStatus.onCooldown).toBe(true);
      expect(secondResult.cooldownStatus.remainingTime).toBeGreaterThan(0);
    });

    it('should handle pet photo easter egg', async () => {
      const userId = mockCtx.from.id;
    ...
    it('should handle empty_plate easter egg', async () => {
      const userId = mockCtx.from.id;
  ...
  describe('Companion Easter Egg (celebration)', () => {
    it('should trigger celebration easter egg AND extract nutrition', async () => {
      const userId = mockCtx.from.id;

      // Mock Claude Vision API response - celebration detection with food
      const mockSceneDetection = {
      ...
      const mockNutritionData = {
        ...
        per_portion: {
          ...
          protein_g: 4,
          fat_total_g: 15,
          carbs_available_g: 50,
          fiber_g: 1,
          // ... other nutrition fields
        },
        notes: 'Estimated from similar birthday cakes'
      };
      ...
      expect(result.success).toBe(true);
      expect(result.data.nutrition_data).toBeTruthy();
  ...
  describe('Empty Packaging (P0 Fix)', () => {
    it('should allow nutrition extraction for empty_packaging when label is visible', async () => {
      const userId = mockCtx.from.id;

      // Mock Claude Vision detecting empty packaging WITH visible nutrition label
      const mockSceneDetection = {
      ...
      const mockNutritionData = {
        ...
        per_portion: {
          ...
          protein_g: 2,
          fat_total_g: 10,
          carbs_available_g: 15,
          fiber_g: 1,
          // ... other fields
        },
        notes: 'Extracted from nutrition label on packaging'
      };
      ...
      // Simulate processImage
      const result = await claudeIntegration.processImage(
        'https://example.com/empty_bag.jpg',
        { userId: 'user123' }
      );

      expect(result.success).toBe(true);
      expect(result.data.nutrition_data).toBeTruthy(); // Nutrition extracted!
    ...
    it('should trigger easter egg for empty_packaging without label', async () => {
      const userId = mockCtx.from.id;
  ...
  describe('No Easter Egg Triggered', () => {
    it('should return null when no easter egg matches', async () => {
      const userId = mockCtx.from.id;
    ...
    it('should not trigger when confidence is too low', async () => {
      const userId = mockCtx.from.id;
  ...
  describe('Cooldown Expiration', () => {
    it('should allow re-trigger after cooldown expires', async () => {
      ...
      // Record with short cooldown (1 second for testing)
      easterEggCooldownManager.recordEasterEggTrigger(
        userId,
        'person_without_food',
        Date.now() - 7 * 24 * 60 * 60 * 1000 - 1000 // Recorded 7 days + 1 second ago
      );
