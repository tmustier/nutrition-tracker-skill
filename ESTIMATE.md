...
2. **Create the dish file and populate data**:
   - Use `scripts/new_dish_from_template.py` to scaffold a per-dish YAML file under `data/dishes` (see CLI helpers below).
   - Fill in ALL available fields: macros, micronutrients, MUFA/PUFA breakdown, qualitative notes.
   - For estimated values, document your reasoning in `assumptions`, `notes`, and `change_log`.
   - Add comprehensive `source.evidence` with URLs and methods used.

3. **Check placement and naming**:
   - Ensure the file lives in the correct venue folder (or `data/dishes/generic`) and that the `name` field matches the index entry.
   - Keep filenames aligned with the `id` (auto-generated by the script).

4. **Validate before committing**:
   - Run `python scripts/validate_data_bank.py data/dishes`
...
5. **Document research findings** (if applicable):
   - Store venue-specific guidance in `data/dishes/venues/<venue-slug>/research-notes.md`.
...
Validate every per-dish YAML file under `data/dishes`. Checks:
...
Usage:
```bash
python scripts/validate_data_bank.py data/dishes
```

### `scripts/new_dish_from_template.py`
Scaffold a new dish file from the shared template. Automatically places the file in the correct folder and refreshes the index.

Usage:
```bash
python scripts/new_dish_from_template.py \\
  --name "Grilled Salmon Fillet (Simple Health Kitchen)" \\
  --dish-slug grilled_salmon_fillet \\
  --venue simple-health-kitchen \\
  --category main \\
  --portion-desc "restaurant portion" \\
  --source-venue "Simple Health Kitchen, Baker Street (London)"
```

For generic items, add `--scope generic` and omit the venue arguments. The script auto-regenerates the index; review the generated file and populate the remaining fields.
...
## Research notes
Venue-specific research notes live alongside dishes: `data/dishes/venues/<venue-slug>/research-notes.md`. Create the file if it does not exist.
